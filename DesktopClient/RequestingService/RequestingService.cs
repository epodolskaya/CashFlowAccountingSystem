...
{
    class RequestingService<T> : IRequestingService<T> where T : StorableEntity
    {
        ...
        public async Task<ICollection<T>> GetAllAsync()
        {
            ...
            return JsonSerializer.Deserialize<ICollection<T>>(await response.Content.ReadAsStringAsync(), new JsonSerializerOptions(JsonSerializerDefaults.Web))!;
        }
        ...
        {
            HttpResponseMessage response = await HttpClient.PostAsync(string.Empty, new StringContent(JsonSerializer.Serialize(createCommand)));
        ...
        {
            HttpResponseMessage response = await HttpClient.PutAsync(string.Empty, new StringContent(JsonSerializer.Serialize(createCommand)));
